...
首版必须具备本地文件、网页、用户选定历史会话、导图、Skill、口述、扫描件和音视频的处理能力；具体采集、解析、OCR、转写及宿主工具依赖须逐类验证，不能未经实现便称可用，也不新增独立模型账号前提。这里的“全部来源能力必需”指产品支持范围；不要求每位创作者都交齐所有类型，当前项目导入计划仍由创作者实际资料与明确选择决定。
...
| 全部必需来源类型 | 本地文件、网页、用户选定历史会话、导图、Skill、口述、扫描件、音视频均有实际验证的处理路径，不要求单个项目交齐所有类型 |
