...
async function main() {
    ...
    if (command === "doctor") {
        const tools = await Promise.all(["node", "xcrun", "ffmpeg"].map(async (program) => {
            try {
                const { run } = await import("./sources.js");
                const version = await run(program, program === "ffmpeg" ? ["-version"] : ["--version"], 10000);
    ...
    if (command === "open") {
        ...
        for (const source of store.sourceList())
            if (source.kind !== "xiaohongshu" && ["queued", "running"].includes(source.status))
                worker(store, source.id);
        output({
