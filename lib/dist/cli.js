...
import { BuddyError, check, hash, optional, read, safeId, withLock, } from "./io.js";
import { publishDraft, startPreview, stopPreview, servePreview } from "./preview.js";
...
const output = (value) => process.stdout.write(`${JSON.stringify({ protocolVersion: "1", ...value }, null, 2)}\n`);
function worker(store, sourceId) {
...
function protocol(store) {
    return {
        ...
        invocation: "buddy call turn_begin|turn_finish --workspace <workspace> --input - < JSON via stdin; use the returned workToken, do not assemble internal envelopes",
        rulesVersion: "1.9",
    };
}
async function call(store, operation, args) {
    if ([
        "project_read",
        "input_pending",
        "input_history",
        ...
        if (operation === "input_pending")
            return { inputs: store.pendingInputs(args.sessionId) };
        if (operation === "input_history")
            return store.inputHistory(args.sessionId, args.offset, args.limit);
        if (operation === "schema_read") {
            const properties = resultJsonSchema.properties;
            const fields = args.fields ?? ["intent", "assessments", "delivery"];
            check(Array.isArray(fields) &&
                fields.every((k) => typeof k === "string" && Object.hasOwn(properties, k)), "SCHEMA_FIELDS", "fields 应为结果顶层字段名数组。");
            return {
                digest: hash(resultJsonSchema),
                fields: Object.fromEntries(fields.map((k) => [k, properties[k]])),
            };
        }
        ...
        if (operation === "validate")
            return {
                ...
                stages: Object.fromEntries(["definition", "knowledge", "methods", "service"].map((s) => [
                    s,
                    gates(store.load(), store, s),
                ])),
            };
        if (operation === "handoff_prepare")
            return (await import("./handoff.js")).handoffStatus(store);
        if (operation === "completion_status")
            return {
        ...
        if (operation === "source_read") {
            ...
            const offset = Number(args.offset ?? 0), limit = Math.min(Number(args.limit ?? 8), 30);
            check(Number.isInteger(offset) && offset >= 0 && Number.isInteger(limit) && limit > 0, "PAGE_ARGUMENT", "来源分页参数无效。");
            return {
                sourceId: source.id,
        ...
        if (operation === "context_read") {
            const item = read(store.path("runtime-work", "items", `${safeId(args.stepId)}.json`));
            const context = read(item.contextRef);
            check(hash(context) === item.contextDigest, "CONTEXT_CHANGED", "工作上下文已被修改。");
            let section = args.section ? context[args.section] : context;
            if (args.path) {
                check(Array.isArray(args.path) &&
                    args.path.every((k) => typeof k === "string" && !["__proto__", "constructor", "prototype"].includes(k)), "CONTEXT_PATH", "path 应是字段名数组。");
                for (const key of args.path) {
                    check(section && typeof section === "object" && Object.hasOwn(section, key), "CONTEXT_PATH", "上下文字段不存在。");
                    section = section[key];
                }
            }
            ...
            if (args.offset !== undefined) {
                const offset = Number(args.offset), limit = Number(args.limit ?? 60000);
                check(Number.isInteger(offset) &&
                    offset >= 0 &&
                    Number.isInteger(limit) &&
                    limit > 0 &&
                    limit <= 120000, "PAGE_ARGUMENT", "上下文分页参数无效。");
                return {
            ...
            check(text.length <= 120000, "CONTEXT_BUDGET", "上下文较长，请按 section、path 或 offset 分页读取，全文不会截断。", { contextRef: item.contextRef, totalCharacters: text.length });
            return { context: section, contextDigest: item.contextDigest };
        }
    }
    return withLock(store.directory, async () => {
        check(typeof args.sessionId === "string", "SESSION_REQUIRED", "写操作需要 open 返回的 sessionId。");
        ...
        if (["turn_begin", "turn_finish", "turn_continue"].includes(operation)) {
            ...
            try {
                if (operation === "turn_begin")
                    return await api.begin(args);
                if (operation === "turn_finish")
                    return await api.finish(args);
        ...
        if (operation === "source_retry") {
            ...
            check(!sourceNeedsHostResult(source), "HOST_MEDIA_REQUIRED", "请由宿主工具补齐音视频结果，以新的 operationId 重新 source_import；旧任务内容已冻结，重复重试不能补出缺失结果。", { source: sourceView(source), recovery: sourcePolicy.mediaRecovery });
            worker(store, source.id);
            return {
        ...
        try {
            if (operation === "turn_prepare")
                return await (args.requestId
                    ...
                    : workflow.prepare(args.sessionId, args.inputId, args.pipeline ?? "interview", args.conflictReview));
            if (operation === "work_complete")
                return await workflow.complete(args.requestId, args);
            if (operation === "turn_commit" || operation === "artifact_confirm") {
                if (args.output) {
                    const directive = await workflow.complete(args.requestId, args);
                    if (directive.directive !== "ready_to_commit")
                        return directive;
                }
                return await workflow.commit(args.requestId, args.commitOperationId ?? args.operationId);
            }
            if (operation === "work_retry")
                return await workflow.retry(args.requestId);
            if (operation === "turn_cancel")
                return workflow.cancel(args.sessionId);
            if (operation === "turn_pause")
                return workflow.pause(args.sessionId, true);
...
async function main() {
    if (values.version) {
        output({
            version: read(fileURLToPath(new URL("../package.json", import.meta.url)))
                .version,
            rulesVersion: "1.9",
        });
        return;
    ...
    if (command === "doctor") {
        const tools = await Promise.all(["node", "xcrun"].map(async (program) => {
            try {
                const { run } = await import("./sources.js");
                const version = await run(program, ["--version"], 10000);
    ...
    if (command === "open") {
        const existingProject = values.workspace
            ? optional(join(resolve(values.workspace), "buddy.json")) : undefined;
        const creationKey = values["creation-key"];
        check(creationKey === undefined || creationKey.trim().length > 0, "CREATION_KEY_EMPTY", "启动重试标识不能为空；无需创作者提供，由宿主保存本次启动身份。");
    ...
    if (command === "call") {
        ...
        try {
            ...
            throw new BuddyError("INPUT_JSON", "输入不是有效 JSON；请通过 --input - 从标准输入传入 JSON 对象。");
        }
        check(args && typeof args === "object" && !Array.isArray(args), "INPUT_JSON", "协议输入须为 JSON 对象。");
...
function workspaceAccess(directory, existing = true) {
