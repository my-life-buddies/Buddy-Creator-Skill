...
async function call(store, operation, args) {
    ...
    return withLock(store.directory, async () => {
        ...
        if (operation === "source_retry") {
            ...
            check(!sourceNeedsHostResult(source), "HOST_SOURCE_REQUIRED", "请由宿主工具补齐资料结果，以新的 operationId 重新 source_import；旧任务内容已冻结，重复重试不能补出缺失结果。", { source: sourceView(source), recovery: sourcePolicy.hostRecovery });
            worker(store, source.id);
            return {
