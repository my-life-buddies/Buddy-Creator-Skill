...
const lines = (value, length = 17) => String(value ?? "").match(new RegExp(`.{1,${length}}`, "gu")) ?? [];
export function serviceDiagram(data, buddyId) {
    const stages = (data.stages ?? {});
    const cardHeight = Math.max(470, ...Object.entries(SERVICE_STAGE_MEANINGS).map(([id, meaning]) => 138 + lines(meaning.detail).length * 21 +
        ["goal", "result", "service", "limit"].reduce((height, key) => height + 48 + lines(stages[id]?.[key]).length * 21, 0)));
    const bottom = 110 + cardHeight, height = bottom + 240;
    const text = (x, y, value, size = 14, color = "#a5b8ab") => `<text x="${x}" y="${y}" font-size="${size}" fill="${color}">${escape(value)}</text>`;
    const cards = ["acquisition", "paid", "maintenance"]
        .map((key, index) => {
        const stage = stages[key] ?? {}, x = 40 + index * 360;
        let y = 173;
        const content = [
            [SERVICE_STAGE_MEANINGS[key].plain, SERVICE_STAGE_MEANINGS[key].detail],
            ["服务目标", stage.goal],
            ["用户得到", stage.result],
            ["提供的服务", stage.service],
            ["范围与限制", stage.limit],
            ...
            .map(([label, value]) => {
            const result = text(x + 24, y, label, 12) +
                "\n" +
                lines(value)
                    .map((line, n) => text(x + 24, y + 25 + n * 21, line, 14, "#e4eee6"))
                    .join("\n");
        ...
        .join("\n");
    const arrow = (path, label, x, y, dashed = false) => `<path d="${path}" fill="none" stroke="#90b895" stroke-width="2" marker-end="url(#arrow)" ${dashed ? 'stroke-dasharray="7 6"' : ""}/>${text(x, y, label, 13, "#c8dec9")}`;
