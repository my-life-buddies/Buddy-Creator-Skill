...
export function artifactStatus(state, id) {
    if (confirmed(state, id))
        return "confirmed";
    if (confirmed(state, id, "accepted"))
        return "accepted";
    if (confirmed(state, id, "rejected"))
        return "rejected";
    return state.confirmations.some((c) => c.objectId === id) ? "revised" : "pending";
}
function objectForTarget(state, id) {
...
function phaseFor(id, stage) {
    if (id?.startsWith("hypothesis.") || id?.startsWith("H"))
        return "候选校准";
    if (id?.startsWith("M") || id?.startsWith("scenario.M"))
        return "基础情境";
    if (id?.startsWith("E") || id?.startsWith("scenario.E"))
        return "拓展情境";
    if (id?.startsWith("T.") || id?.startsWith("transition."))
        return "用户路径";
    if (id === "service.blueprint")
        return "服务模式确认";
    if (id === "R00")
        return "本次优化";
    if (id === "S00")
        return "规划方式";
    if (id && /^(definition|knowledge|methods|service)\.\d+$/.test(id))
        return "手册确认";
...
export function projectPreview(state, delivery) {
    const artifacts = Object.values(state.artifacts).map((a) => ({
    ...
    const requested = delivery?.confirmationTarget?.stage === state.stage
        ...
        : [];
    const pending = (a) => ["pending", "revised"].includes(a.status);
    ...
    const stages = STAGES.map((stage, index) => {
        ...
        const groups = [];
        const group = (id, title, targets) => groups.push({ id, title, topics: targets.map(topic) });
        if (stage === "methods") {
