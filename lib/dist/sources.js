...
function validateHostMediaResult(request) {
    ...
    check(typeof request.uri === "string" && request.uri.trim(), "SOURCE_LOCATION", "请同时提供创作者选定的本地音视频原件路径，供结果关联与归档。");
    check(typeof result.tool === "string" && result.tool.trim() &&
        ["complete", "partial"].includes(result.coverage), "HOST_MEDIA_RESULT", "请如实记录实际使用的宿主工具及 complete / partial 处理范围。");
    check(Array.isArray(result.parts) && result.parts.length && result.parts.every((part) => part && typeof part.text === "string" && part.text.trim() &&
        typeof part.locator === "string" && part.locator.trim()), "HOST_MEDIA_RESULT", "宿主结果需包含实际取得的文字和位置；无时间戳时使用段落位置并注明 time=unavailable，不虚构定位。");
    check(result.notes === undefined || (Array.isArray(result.notes) && result.notes.every((note) => typeof note === "string")), "HOST_MEDIA_RESULT", "处理限制应以 notes 文字列表保存。");
    return result;
}
...
export async function nativeMedia(store, command, path) {
    check(process.platform === "darwin", "MACOS_REQUIRED", "扫描件 OCR 使用 macOS 系统能力。");
    const source = fileURLToPath(new URL("../native/Media.swift", import.meta.url));
    const cacheIdentity = {
        command,
        inputHash: hash(readFileSync(path)),
        helperHash: hash(readFileSync(source)),
    };
    const resultPath = store.path("source-jobs", "media-results", `${hash(cacheIdentity)}.json`);
    const cached = optional(resultPath);
    if (cached) {
        check(hash(cached.identity) === hash(cacheIdentity) &&
            cached.digest === hash(cached.parts), "MEDIA_RESULT_CHANGED", "已保存的媒体识别结果被改动，请保留现场核对。");
        return cached.parts;
    }
    const directory = store.path(".tools", hash(readFileSync(source)).slice(0, 16));
    const executable = join(directory, "BuddyMedia");
    if (!existsSync(executable)) {
        mkdirSync(directory, { recursive: true, mode: 0o700 });
        const info = fileURLToPath(new URL("../native/Info.plist", import.meta.url));
    ...
    const output = JSON.parse(await run(executable, [command, path], 1800000));
    check(output.parts?.some((p) => p.text.trim()), "EMPTY_TRANSCRIPT", "系统未识别到有效文字，不能标为已读取。");
...
async function parseLocal(store, path, request) {
    check(existsSync(path), "FILE_MISSING", "找不到选定的来源文件。", { path });
    const kind = request.kind, extension = extname(path).toLowerCase();
    let warnings = [];
        ...
        function visit(directory) {
            for (const name of readdirSync(directory)) {
                ...
                if (stat.isDirectory()) {
                    ...
                    check(Object.keys(files).length < 5000, "SOURCE_SIZE", "Skill 文件数超过5000，请限定来源范围。");
    ...
    const original = readFileSync(path), name = basename(path);
    let parts = [];
    let parser = "text-v1";
    const extras = [];
    if (kind === "history") {
        ...
        warnings.push("音视频内容来自宿主工具的处理结果；仅在已提供的定位和覆盖范围内引用，归纳仍需创作者校准。", ...(result.notes ?? []));
    }
        ...
        if (scanned) {
            ...
            parts = parts.map((p) => p.text.trim() ? p : (ocr.find((o) => o.locator === p.locator) ?? p));
        ...
        if (extension === ".xmind") {
            ...
            check(entries["content.json"] || entries["content.xml"], "MINDMAP_FORMAT", "XMind 文件缺少 content.json / content.xml。");
            data = entries["content.json"]
                ? JSON.parse(strFromU8(entries["content.json"]))
                : new XMLParser({ ignoreAttributes: false }).parse(strFromU8(entries["content.xml"]));
        }
        else
            data =
                extension === ".json"
                    ? JSON.parse(original.toString("utf8"))
                    : new XMLParser({ ignoreAttributes: false }).parse(original.toString("utf8"));
        function walk(value, path) {
            ...
            for (const [key, v] of Object.entries(value)) {
                if (["title", "@_TEXT", "@_text", "#text"].includes(key) &&
                    typeof v === "string")
        ...
        check([
            ...
            kind === "oral" ||
            kind === "skill", "UNSUPPORTED_FORMAT", "该文件格式尚未提供解析器。", { extension });
        parts = [{ text: original.toString("utf8"), locator: "document" }];
    }
    check(parts.some((p) => p.text.trim()), "EMPTY_SOURCE", "没有解析到可用内容，不能标为已读取。");
...
export async function readableWeb(url) {
    ...
    check(response.ok, "WEB_FETCH_FAILED", `网页返回 ${response.status}，内容没有导入。`);
    check(Number(response.headers.get("content-length") ?? 0) <= 20000000, "SOURCE_SIZE", "网页超过20MB。");
    const chunks = [];
    let size = 0;
...
export function enqueueSource(store, request) {
    ...
    if (isHostMedia(request.kind))
        validateHostMediaResult(request);
    else
        check(request.hostResult === undefined, "HOST_RESULT_KIND", "hostResult 仅用于 audio / video 来源。");
    if (request.kind === "webpage" && request.uri)
...
export async function processSource(store, sourceId) {
    return withLock(store.path("source-jobs", "locks", safeId(sourceId)), async () => {
        ...
        try {
            ...
            check(!isHostMedia(request.kind) || request.hostResult?.coverage === "complete", "HOST_MEDIA_INCOMPLETE", "宿主仅提供了部分音视频结果，已保存原件与结果，但尚未完整处理。请补齐后以新的 operationId 导入，不重复重试相同的部分结果。", { recovery: sourcePolicy.mediaRecovery });
