...
export async function nativeMedia(store, command, path, locale = "zh-CN") {
    check(process.platform === "darwin", "MACOS_REQUIRED", "扫描件和本地转写使用 macOS 系统能力。");
    ...
    const output = JSON.parse(await run(executable, [command, path, locale], 1800000));
...
async function parseLocal(store, path, request) {
    ...
    if (kind === "history") {
        ...
        if (kind === "video") {
            ...
            const frames = store.path(".tools", `frames-${hash(original).slice(0, 24)}`);
            ...
            extras.push({
                ...
                data: Buffer.from(JSON.stringify({
                    sampling: "one frame per 10 seconds, midpoint selection; first frame fallback for short videos",
                    durationSeconds: Number(probe.format.duration),
                    frames: names.length,
                })),
            });
            warnings.push("画面按每10秒一帧抽样归档并识别文字；非文字视觉含义未自动归纳，原视频完整保留，必要时需结合原片校准。");
        }
        if (hasAudio) {
            try {
                parts.push(...(await nativeMedia(store, "transcribe", audioPath, request.locale)));
            }
            catch (error) {
                if (kind !== "video" ||
        ...
        warnings.push("自动转写可能存在识别误差；知识归纳须引用时间位置并由创作者校准。");
    }
