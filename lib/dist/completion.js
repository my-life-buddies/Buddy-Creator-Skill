...
function ready(store, state) {
    if (state.paused || store.session()?.paused || !handoffStatus(store, state).ready)
        return false;
    ...
    while (revision) {
        ...
        if (record.delivery?.inputId)
            completed.add(record.delivery.inputId);
        revision = record.parent;
    }
    for (const event of jsonFiles(store.path("deliveries", "events")))
        if (event.delivery.inputId)
            completed.add(event.delivery.inputId);
    return !jsonFiles(store.path("inputs", "tickets")).some((ticket) => !completed.has(ticket.inputId) && existsSync(store.path("inputs", "records", `${safeId(ticket.inputId)}.json`)));
}
function paths(store, revision) {
    return { directory: store.path("deliverables", safeId(revision)),
        record: store.path("deliverables", "status", `${safeId(revision)}.json`) };
}
function intact(directory, record, full) {
    try {
        const bytes = readFileSync(join(directory, "MANIFEST.json"));
        if (hash(bytes) !== record.manifestHash)
            return false;
        const manifest = JSON.parse(bytes.toString());
        if (manifest.revision !== record.revision || manifest.buddyId !== record.buddyId || !manifest.complete)
            return false;
        return manifest.files.every((file) => existsSync(join(directory, file.path)) && (!full || (readFileSync(join(directory, file.path)).length === file.bytes &&
...
export async function ensureCompletion(store) {
    return withLock(store.path("deliverables", ".finalize-lock"), () => {
        ...
        const previous = optional(recordPath);
        ...
        const base = { version: 1, buddyId: state.buddyId, revision: state.revision,
        ...
        try {
            ...
            for (const [path, bytes] of Object.entries(files))
                atomic(join(staging, path), Buffer.from(bytes));
            const record = { ...base, status: "ready", fileCount: Object.keys(files).length,
                manifestHash: hash(Buffer.from(files["MANIFEST.json"])) };
            if (!intact(staging, record, true))
                throw new Error("本地成果未完整写入，请重试生成。");
            // Preserve changed derived files for inspection; original interview records are never rewritten.
            if (existsSync(directory))
                renameSync(directory, `${directory}.previous-${randomUUID()}`);
            ...
            const record = { ...base, status: "failed", error: error instanceof Error ? error.message : String(error) };
            json(recordPath, record);
            return record;
        }
    });
...
export function completionSnapshot(store) {
    const state = store.load();
    if (!ready(store, state))
        return undefined;
    const { directory, record: recordPath } = paths(store, state.revision);
    const record = optional(recordPath);
    if (!record || record.revision !== state.revision || record.buddyId !== state.buddyId)
        return undefined;
    const valid = record.status === "ready" && intact(directory, record, false);
    return { revision: state.revision, status: valid ? "ready" : "failed",
...
export async function withCompletion(store, directive) {
    try {
