...
import type { Input, ProjectState, WorkEnvelope, WorkItem, WorkResult, Stage } from "./types.js";
...
type Handle = { sessionId: string; requestId: string; item: WorkItem };
const commitId = (requestId: string) => `fast_commit_${hash(requestId).slice(0, 28)}`;
const workId = (token: string) => `fast_work_${hash(token).slice(0, 28)}`;
...
export function stageRules(stages: Stage[]) {
...
export function compactWork(
  store: Store,
  directive: Record<string, any>,
  sessionId: string,
  knownRulesDigest?: string,
) {
  if (directive.directive !== "host_work") return directive;
  const item = directive.workItem as WorkItem;
  const c = read<Record<string, any>>(item.contextRef);
  check(hash(c) === item.contextDigest, "CONTEXT_CHANGED", "工作上下文已改变。");
  const state = c.state as ProjectState;
  const stages = [
    ...new Set(
      Object.values(c.targetCards as Record<string, { stage: Stage }>).map((v) => v.stage),
    ),
  ];
  const rules = stageRules(stages);
  const replyIds = new Set<string>([
    ...(c.replyTo?.confirmationTarget?.objects ?? []).map((v: { id: string }) => v.id),
  ...
  return {
    ...
    context: {
      ...
      targets: Object.fromEntries(
        Object.keys(c.targetCards).map((id) => {
    ...
    outputContract: {
      ...
      assessment:
        "{targetId,status,summary,gaps:[],evidence:[{type:'input',id,hash,quote}]}；引用本轮 input 或已保存证据。",
      delivery:
        "{text,mode?,questionTargetId?,confirmationObjectIds?,confirmationScope?}；正常推进必须在 text 内明确提出下一问或版本确认，并绑定对应字段；确认与采访二选一。只有纯答疑、真实暂停、完成或确无可推进内容时才可无绑定。",
      ...
      readMore:
        "需要产物、确认、重大改向等字段时，schema_read 可按 fields 返回规范；需要历史或正文时 context_read 按 section/path 读取。不修改或重发未变字段。",
    },
    fullContext: {
      stepId: item.stepId,
...
export class TurnAPI {
  ...
  async next(requestId: string, sessionId: string, knownRulesDigest?: string) {
  ...
  async begin(args: Begin) {
    const session = this.store.requireSession(args.sessionId);
    check(
      typeof args.clientKey === "string" &&
        args.clientKey.length > 0 &&
        args.clientKey.length <= 256,
      "INPUT_IDENTITY_REQUIRED",
      "clientKey 必须是本条消息稳定且唯一的标识。",
    );
    check(
      typeof args.raw === "string" && args.raw.trim() && args.raw.length <= 200000,
      "INPUT_SIZE",
      "原话须为非空文本且不超过200,000字符。",
    );
    const body = {
    ...
    const path = this.store.path(
      "runtime-work",
      "begins",
      `${hash({ sessionId: args.sessionId, clientKey: args.clientKey })}.json`,
    );
    const previous = optional<{
      digest: string;
      inputId: string;
      requestId: string;
      epoch: number;
    ...
    check(
      !previous || previous.digest === hash(body),
      "IDEMPOTENCY_CONFLICT",
      "相同消息身份的原话、回复对象或处理范围不同。",
    );
    check(
      !previous || previous.epoch === session.epoch,
      "EXECUTION_REPLACED",
      "旧消息所属执行已被替代，请恢复当前执行。",
    );
    ...
    if (previous) {
      ...
      if (receipt) return withCompletion(this.store, { receipt, ...this.store.deliveryDirective(receipt.deliveryId) });
    }
    if (session.activeRequestId && session.activeRequestId !== previous?.requestId) {
      ...
      check(
        Boolean(pending.receipt),
        "TURN_ACTIVE",
        "上一轮尚未完成，请先 turn_resume 或明确取消。",
    ...
    const d = await this.workflow.prepare(
      args.sessionId,
      ticket.inputId,
      args.pipeline,
      args.conflictReview,
  ...
  async finish(args: {
    ...
    const h = read<Handle>(
      this.store.path("runtime-work", "handles", `${safeId(args.workToken)}.json`),
    );
    const s = this.store.requireSession(args.sessionId);
    check(
      h.sessionId === s.id && h.item.operationEpoch === s.epoch,
      "EXECUTION_REPLACED",
      "工作凭据不属于当前连接或执行代际。",
    );
    ...
    const operation = optional<{ digest: string }>(
      this.store.path("runtime-work", "operations", `${envelope.operationId}.json`),
    );
    check(
      !operation || operation.digest === hash(envelope),
      "IDEMPOTENCY_CONFLICT",
      "已接受的工作凭据不能提交不同结果。",
    );
