...
import type { Ref, SourceManifest, ProjectState } from "./types.js";

export function handoffStatus(store: Store, state = store.load()) {
  return {
    revision: state.revision,
    ready: STAGES.every(
      (s) =>
        bookConfirmed(state, s) && gates(state, store, s).every((g) => g.pass),
    ),
...
export function collectDeliverables(store: Store, state: ProjectState = store.load()) {
  const status = handoffStatus(store, state);
  check(
    status.ready,
    "HANDOFF_NOT_READY",
    "四份手册尚未完成确认，或来源与业务门槛未通过。",
    { status },
  );
  ...
  put(
    "service-diagram.svg",
    serviceDiagram(
      state.artifacts["service.blueprint"]?.data ?? {},
      state.buddyId,
    ),
  );
  ...
  for (const b of books) put(`booklets/${b.stage}.md`, b.text);
  put(
    "BUDDY_MANUAL.md",
    books.map((b) => `# ${b.stage}\n\n${b.text}`).join("\n---\n\n"),
  );
  ...
  for (const ref of refs) if (ref.type === "input") inputIds.add(ref.id);
  for (const target of Object.values(state.targets))
    for (const id of target.answerInputIds) inputIds.add(id);
  for (const c of state.cycles) {
    if (c.inputId) inputIds.add(c.inputId);
  ...
  while (revision) {
    ...
    if (r.delivery) {
      ...
      if (r.delivery.inputId) inputIds.add(r.delivery.inputId);
    }
    revision = r.parent;
  }
  ...
  for (const id of inputIds) {
    ...
    if (input.replyToDeliveryId) {
      ...
      if (reply.inputId) inputIds.add(reply.inputId);
    }
  }
  ...
  for (const id of new Set([
    ...
    for (const file of source.files) {
      check(
        existsSync(store.path(file.path)),
        "SOURCE_ASSET_MISSING",
        "来源原件或派生文件缺失，不能生成完整本地成果。",
        { path: file.path },
      );
      const data = readFileSync(store.path(file.path));
      check(
        hash(data) === file.hash && data.length === file.bytes,
...
export function exportHandoff(store: Store, operationId: string, options: { revision?: string } = {}) {
  safeId(operationId);
  ...
  if (existsSync(previousPath)) {
    const previous = read<typeof receipt>(previousPath);
    check(
