...
import type { WorkItem, WorkEnvelope } from "./types.js";
...
function acceptedSteps(store: Store) {
  return new Set(
    jsonFiles<{ resultRef: string }>(
      store.path("runtime-work", "operations"),
    ).map(
      (o) =>
        read<WorkEnvelope>(
          store.path("runtime-work", "results", `${safeId(o.resultRef)}.json`),
        ).stepId,
    ),
  );
}
export function publishDraft(
  ...
  check(
    item.operationEpoch === session.epoch &&
      item.requestId === session.activeRequestId &&
      item.baseRevision === store.load().revision &&
      args.contextDigest === item.contextDigest,
    "STALE_DRAFT",
    "这份草稿的工作项已失效。",
  );
  check(
    !session.paused && !acceptedSteps(store).has(item.stepId),
    "STALE_DRAFT",
    "这项工作已提交结果或已暂停，不能继续发布草稿。",
  );
  check(
    Number.isInteger(args.sequence) &&
      args.sequence >= 0 &&
      args.markdown.length <= 64000,
    "DRAFT_BUDGET",
    "草稿序号无效或单个可见块超过64,000字符。",
  );
  const draft: Draft = {
...
export function previewSnapshot(store: Store) {
  ...
  const drafts = jsonFiles<Draft>(store.path("drafts"))
    .filter(
      (d) =>
        d.requestId === session?.activeRequestId &&
  ...
  return {
    ...
    continuation: sources
      .some(
        (s) =>
          s.status === "ready" && state.sources[s.id]?.version !== s.version,
      )
      ? "资料已就绪，回到主对话说“继续”。"
      : undefined,
  };
}
type PreviewEndpoint = { version: 1; workspaceId: string; port: number; token: string };
type PreviewServerInfo = { pid: number; url: string; token: string; workspaceId?: string; runtimeVersion?: string };
const runtimeVersion = JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8")).version as string;
const validPort = (value: unknown): value is number => Number.isInteger(value) && Number(value) > 0 && Number(value) <= 65535;
const validToken = (value: unknown): value is string => typeof value === "string" && /^[a-f0-9]{48}$/.test(value);

function serverRecord(store: Store): PreviewServerInfo | undefined {
  const info = optional<PreviewServerInfo>(store.path("preview-server.json"));
  if (!info) return;
  check(Number.isSafeInteger(info.pid) && info.pid > 0 && validToken(info.token),
    "PREVIEW_ADDRESS_INVALID", "预览进程记录无效，请检查当前搭子的 preview-server.json；不会自动换地址。");
  ...
  check(match && validPort(Number(match[1])) && match[2] === info.token &&
    (!info.workspaceId || info.workspaceId === store.load().workspaceId),
    "PREVIEW_ADDRESS_INVALID", "预览地址不属于当前搭子的本机服务，已保留记录，不会访问该地址。");
  return info;
}

function savedEndpoint(store: Store): PreviewEndpoint | undefined {
  const workspaceId = store.load().workspaceId;
  const endpoint = optional<PreviewEndpoint>(store.path("preview-endpoint.json"));
  if (endpoint) {
    check(endpoint.version === 1 && endpoint.workspaceId === workspaceId && validPort(endpoint.port) && validToken(endpoint.token),
      "PREVIEW_ADDRESS_INVALID", "固定预览地址无效或属于其他搭子，请检查 preview-endpoint.json；不会覆盖原记录。");
    return endpoint;
  }
...
async function previewAlive(store: Store, info: PreviewServerInfo) {
  try {
    const health = await fetch(`${info.url}api/health`, { signal: AbortSignal.timeout(1500), redirect: "error" });
    if (health.ok) {
      const identity = await health.json() as { workspaceId: string; pid: number };
      check(identity.workspaceId === store.load().workspaceId && identity.pid === info.pid,
        "PREVIEW_IDENTITY_MISMATCH", "这个端口上的服务不是所记录的当前搭子预览，未接管或停止它。");
      return true;
    }
    ...
    if (health.status === 404 && !info.workspaceId) {
      ...
      if (response.ok) {
        const view = await response.json() as { buddyId: string };
...
export async function servePreview(store: Store, port = 0) {
  return withLock(store.path(".preview-binding"), async () => {
    ...
    const token = endpoint?.token ?? randomBytes(24).toString("hex"),
      base = `/p/${token}/`;
    const workspaceId = store.load().workspaceId;
    ...
    const server = createServer((req, res) => {
      ...
      res.setHeader(
        ...
        "default-src 'self'; script-src 'self'; connect-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; frame-ancestors 'none'",
      );
      if (req.method !== "GET") {
        res.writeHead(405);
    ...
    try {
      json(store.path("preview-endpoint.json"), { version: 1, workspaceId, port: address.port, token } satisfies PreviewEndpoint);
      json(store.path("preview-server.json"), { pid: process.pid, url, token, workspaceId, runtimeVersion } satisfies PreviewServerInfo);
...
export async function startPreview(store: Store, openBrowser = true) {
  return withLock(store.path(".preview-lifecycle"), async () => {
    ...
    if (info && !await previewAlive(store, info)) info = undefined;
    ...
    if (!info) {
      ...
      for (let i = 0; i < 60; i++) {
        ...
        if (child.exitCode !== null) {
          let failure: { error?: { code?: string; message?: string; details?: unknown } } = {};
          try { failure = JSON.parse(startupOutput); } catch {}
      ...
      check(
        ...
        "本地预览未能启动；请运行 buddy serve 查看具体错误。",
      );
      } finally {
        if (!info && child.exitCode === null) child.kill("SIGTERM");
...
export async function stopPreview(store: Store) {
  return withLock(store.path(".preview-lifecycle"), async () => {
    savedEndpoint(store);
    const info = serverRecord(store);
    if (!info) return { stopped: true, status: "not_running" };
...
async function stopVerifiedPreview(info: PreviewServerInfo) {
