...
export class SourceWorkflow {
      ...
      .addNode("source_process", async ({ sourceId }) => {
        ...
        if (source.status !== "ready")
          throw new BuddyError(
            ...
            source.error ?? "资料处理未完成。",
            { sourceId, jobId: source.jobId, version: source.version },
          );
  ...
  async run(sourceId: string): Promise<SourceManifest> {
    const source = this.store.source(sourceId);
    if (source.status === "ready" && source.kind === "xiaohongshu") return source;
    ...
    return withLock(
      this.store.path("source-jobs", "workflow-locks", sourceId),
      async () => {
        let saved = await this.graph.getState(config);
        if (saved.values.sourceId) {
          check(
            saved.values.sourceId === sourceId,
            "SOURCE_WORK_IDENTITY",
            "资料检查点与当前任务不一致。",
          );
          if (saved.next.length) await this.graph.invoke(null, config);
        } else await this.graph.invoke({ sourceId }, config);
        saved = await this.graph.getState(config);
        const complete = this.store.source(
          sourceId,
          saved.values.version as string,
        );
        check(
          complete.status === "ready" && !saved.next.length,
          "SOURCE_WORK_INCOMPLETE",
          "资料工作还未完成，已保留进度。",
        );
        return complete;
      },
