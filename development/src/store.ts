...
export class Store {
  ...
  load(): ProjectState {
    return this.revision(read<{ revision: string }>(this.path("HEAD.json")).revision).state;
  }
  revision(revision: string): Revision {
    const record = read<{ digest: string; body: Revision }>(
      this.path("revisions", `${safeId(revision)}.json`),
    );
    check(
      record.digest === hash(record.body),
      "WORKSPACE_TAMPERED",
      "项目文件与记录摘要不一致。已停止写入，请保留现场核对。",
      { revision },
    );
    return record.body;
  }
  session() {
    return optional<Session>(this.path("session.json"));
  }
  ...
  connect(host: Session["host"], sessionId?: string, takeover = false): Session {
    const existing = this.session();
    if (existing && sessionId === existing.id && !takeover) return existing;
    check(
      !existing || existing.host === host || takeover,
      "SESSION_OWNED",
      "项目正在由另一个宿主接续；使用明确的接替操作继续。",
      { host: existing?.host },
    );
    if (existing && existing.host === host && !takeover) return existing;
    const s: Session = {
      id: id("session"),
      host,
  ...
  dialogue(): Dialogue {
    return optional<Dialogue>(this.path("dialogue.json")) ?? {};
  }
  reserve(sessionId: string, clientKey: string, replyToDeliveryId?: string): Ticket {
    this.requireSession(sessionId);
    check(
      clientKey.length > 0 && clientKey.length <= 256,
      "INPUT_IDENTITY_REQUIRED",
      "先为这条消息提供可恢复的 clientKey。",
    );
    const path = this.path("inputs", "tickets", `${hash({ sessionId, clientKey })}.json`);
    const previous = optional<Ticket>(path);
    if (previous) {
      check(
        replyToDeliveryId === undefined || previous.replyToDeliveryId === replyToDeliveryId,
    ...
    const ordinal =
      (optional<{ ordinal: number }>(this.path("inputs", "sequence.json"))?.ordinal ?? 0) + 1;
    ...
    const ticket = {
      ...
      createdAt: now(),
      replyToDeliveryId:
        replyToDeliveryId ?? this.dialogue().questionDeliveryId ?? this.dialogue().activeDeliveryId,
    };
  ...
  pendingInputs(sessionId: string) {
    ...
    let revision: string | undefined = this.load().revision;
    while (revision) {
      const record = this.revision(revision);
      if (record.delivery?.inputId) done.add(record.delivery.inputId);
      revision = record.parent;
    }
    for (const event of jsonFiles<{ delivery: Delivery }>(this.path("deliveries", "events")))
      if (event.delivery.inputId) done.add(event.delivery.inputId);
    return jsonFiles<Ticket>(this.path("inputs", "tickets"))
      .filter((t) => t.sessionId === sessionId && !done.has(t.inputId))
      .sort((a, b) => a.ordinal - b.ordinal)
      .map((t) => ({
        ...t,
        input: optional<Input>(this.path("inputs", "records", `${t.inputId}.json`)),
      }));
  }
  inputHistory(sessionId: string, offset = 0, limit = 20) {
    this.requireSession(sessionId);
    check(
    ...
    const tickets = jsonFiles<Ticket>(this.path("inputs", "tickets")).sort(
  ...
  record(sessionId: string, token: string, raw: string, annotation?: Input["annotation"]): Input {
    this.requireSession(sessionId);
    const ticket = read<Ticket>(this.path("inputs", "tokens", `${safeId(token)}.json`));
    check(ticket.sessionId === sessionId, "INPUT_SESSION", "输入凭据不属于当前连接。");
    check(
      raw.trim() && raw.length <= 200000,
      "INPUT_SIZE",
      "单次输入须为非空文本，且不超过 200,000 字符；更长内容请作为资料导入。",
    );
    const path = this.path("inputs", "records", `${ticket.inputId}.json`);
    const previous = optional<Input>(path);
    if (previous) {
      check(
        previous.raw === raw && hash(previous.annotation) === hash(annotation),
        "IDEMPOTENCY_CONFLICT",
        "同一输入凭据不能提交不同内容。",
      );
  ...
  input(inputId: string) {
    const input = read<Input>(this.path("inputs", "records", `${safeId(inputId)}.json`));
    check(
      input.id === inputId && input.hash === hash(input.raw),
      "INPUT_CHANGED",
      "原始输入记录被改动，请保留现场核对。",
    );
    return input;
  ...
  source(sourceId: string, version?: string): SourceManifest {
    const selected =
      version ??
      read<{ version: string }>(this.path("sources", safeId(sourceId), "HEAD.json")).version;
    const manifest = read<SourceManifest>(
      this.path("sources", safeId(sourceId), "versions", `${safeId(selected)}.json`),
    );
    check(
      manifest.id === sourceId && manifest.version === selected,
      "SOURCE_IDENTITY",
      "来源版本不匹配。",
    );
    if (manifest.status === "ready") {
      ...
      check(
        selected === expected &&
          ...
          manifest.chunks.every((c) => hash(c.text) === c.hash),
        "SOURCE_CHANGED",
        "来源清单或全文分块被改动，请保留现场核对。",
      );
    }
    return manifest;
  }
  ...
  delivery(deliveryId: string): Delivery {
    return read<Delivery>(this.path("deliveries", "records", `${safeId(deliveryId)}.json`));
  }
  shown(deliveryId: string) {
  ...
  isDeliveryValid(d: Delivery) {
    const state = this.load(),
      dialogue = this.dialogue();
    if (existsSync(this.path("runtime-work", "cancelled", `${safeId(d.requestId)}.json`)))
      return false;
    if (dialogue.activeDeliveryId !== d.id) return false;
    if (dialogue.latestInputId && dialogue.latestInputId !== d.inputId) return false;
    if (d.kind === "content" && d.revision !== state.revision) return false;
  ...
  deliveryDirective(deliveryId?: string) {
    if (!deliveryId) return { directive: "await_user" };
  ...
  findReceipt(operationId: string): Receipt | undefined {
    ...
    while (revision) {
      ...
      if (r.receipt?.operationId === operationId) return r.receipt;
      revision = r.parent;
    }
    return optional<{ receipt: Receipt }>(
      this.path("deliveries", "events", `${safeId(operationId)}.json`),
    )?.receipt;
  }
  publishCommittedDelivery(d: Delivery) {
    ...
    if (dialogue.latestInputId && dialogue.latestInputId !== d.inputId) return;
    if (d.kind === "content" && this.load().revision !== d.revision) return;
    json(this.path("dialogue.json"), {
      ...dialogue,
      activeDeliveryId: d.id,
      questionDeliveryId:
        d.question || d.confirmationTarget
          ? d.id
          : d.kind === "explanation"
  ...
  reconcile() {
    ...
    const session = this.session(),
      dialogue = this.dialogue();
    const event = jsonFiles<{ receipt: Receipt; delivery: Delivery }>(
      this.path("deliveries", "events"),
    ).find(
      (e) =>
        e.receipt.requestId === session?.activeRequestId &&
  ...
  commit(
    ...
    check(
      this.load().revision === baseRevision,
      "STALE_REVISION",
      "正式内容已变化，请基于当前版本重新准备。",
    );
    ...
    for (const a of Object.values(state.artifacts)) if (!a.revision) a.revision = revision;
    ...
    const body: Revision = { parent: baseRevision, state, receipt, delivery };
...
export function locateWorkspace(
  buddyId: string,
  options: { home?: string; root?: string; workspace?: string } = {},
) {
  safeId(buddyId);
  ...
  if (indexed && existsSync(join(indexed, "buddy.json"))) candidates.add(realpathSync(indexed));
  for (const registeredRoot of registry.roots) {
    ...
    for (const child of readdirSync(registeredRoot, {
      ...
      const meta = optional<{ buddyId: string }>(join(candidate, "buddy.json"));
      if (meta?.buddyId === buddyId) candidates.add(realpathSync(candidate));
    }
  }
  if (options.workspace && existsSync(join(resolve(options.workspace), "buddy.json"))) {
    check(
      read<{ buddyId: string }>(join(resolve(options.workspace), "buddy.json")).buddyId === buddyId,
      "BUDDY_MISMATCH",
      "指定目录属于另一位 Buddy。",
    );
  ...
  const explicit =
    options.workspace && existsSync(join(resolve(options.workspace), "buddy.json"))
  ...
  const chosen =
    ...
    (registry.preferred?.[buddyId] && candidates.has(registry.preferred[buddyId])
      ...
      : undefined);
  if (explicit) registry.preferred = { ...registry.preferred, [buddyId]: explicit };
  check(
    chosen || candidates.size <= 1,
    "DUPLICATE_WORKSPACES",
    "发现多个相同 buddyid 的目录，请保留现场并明确选择需要恢复的目录。",
    { candidates: [...candidates] },
  );
  const directory =
    chosen ?? [...candidates][0] ?? resolve(options.workspace ?? join(root, buddyId));
