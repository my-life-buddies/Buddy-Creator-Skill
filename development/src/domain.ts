...
export function bookIds(stage: Stage) {
  return CHAPTERS[stage].map((_, i) => `${stage}.${i + 1}`);
}
export function bookConfirmed(state: ProjectState, stage: Stage) {
  return bookIds(stage).every((id) => confirmed(state, id));
}
export function verifyRef(store: Store, state: ProjectState, ref: Ref) {
  let text = "";
  if (ref.type === "input") {
    ...
    const chunk = source.chunks.find(
      (c) => c.hash === ref.hash && c.locator === ref.locator,
    );
    check(
      chunk && hash(chunk.text) === chunk.hash,
      "EVIDENCE_LOCATOR",
      "来源引用必须指向确切的全文块及位置。",
    );
    text = chunk.text;
...
function inputEvidence(
  ...
  check(
    refs.some(
      (r) => r.type === "input" && r.id === input.id && r.quote?.trim(),
    ),
...
export function gates(
  ...
  if (stage === "definition")
    return Object.entries(CARDS)
      ...
      .map(([id, c]) => ({
        label: c.title,
        pass: ["sufficient", "uncertain", "skipped", "exhausted"].includes(
          state.targets[id]!.status,
        ),
      }));
  if (stage === "knowledge") {
    ...
    return [
      ...
      },
      {
        label: "全部所选来源真实可用",
        pass: sources.every(
          (s) => s?.status === "ready" && s.chunks.length > 0,
        ),
      },
      {
        label: "每类必需来源有可用内容",
        pass: state.sourcePlan.requiredKinds.every((k) =>
          sources.some((s) => s?.kind === k && s.status === "ready"),
        ),
      },
    ];
  }
  if (stage === "methods") {
    const hypotheses = Object.values(state.artifacts).filter(
      (a) => a.kind === "hypothesis",
    );
    return [
      {
...
function serviceRulesMatch(data: Record<string, unknown>) {
  return hash(data.platformRules) === hash(SERVICE_RULES);
}
function serviceGates(state: ProjectState) {
  const data = state.artifacts["service.blueprint"]?.data ?? {};
  const stages = (data.stages ?? {}) as Record<string, Record<string, unknown>>;
  const transitions = (data.transitions ?? {}) as Record<
    string,
    Record<string, unknown>
  >;
  const loop = (data.serviceLoop ?? {}) as Record<string, string>;
  const contract = (data.acquisitionContract ?? {}) as Record<string, unknown>;
  const attest = (data.semanticChecks ?? {}) as Record<
    string,
    { pass: boolean; evidence: Ref[] }
  >;
  const text = (v: unknown) => typeof v === "string" && v.trim().length > 0;
  const list = (v: unknown) =>
    Array.isArray(v) && v.length > 0 && v.every(text);
  const known = (key: string) =>
    attest[key]?.pass === true && Boolean(attest[key]?.evidence?.length);
  const filled = (o: Record<string, unknown> | undefined, keys: string[]) =>
    Boolean(o) && keys.every((k) => text(o![k]));
  return [
    {
      label: "持续价值",
      pass:
        text(data.valueStatement) &&
    ...
    },
    {
      label: "付费可交付",
      pass:
        filled(stages.paid, ["result", "service", "limit"]) &&
    ...
    },
    {
      label: "停付边界",
      pass:
        filled(stages.maintenance, ["service", "limit"]) &&
        (data.maintenanceContract as Record<string, unknown> | undefined)?.basicConversation === true &&
        text((data.maintenanceContract as Record<string, unknown> | undefined)?.allowedQuestions) &&
    ...
    },
    {
      label: "单档订阅契约",
      pass:
        Boolean(subscriptionPeriod(data.billingCycle)) &&
...
export function canDraft(state: ProjectState, store: Store, stage: Stage) {
  return (
    gates(state, store, stage).every((g) => g.pass) &&
    (stage !== "service" || confirmed(state, "service.blueprint"))
  );
}
function canAsk(state: ProjectState, store: Store, targetId: string) {
  const card = CARDS[targetId],
    target = state.targets[targetId];
  check(card && target, "UNKNOWN_TARGET", "未知采访目标。", { targetId });
  check(card.stage === state.stage, "STAGE_SCOPE", "不能提前采访其他阶段。");
  if (targetId === "K02")
    check(
      !state.sourcePlan.discoveryClosed,
      "SOURCE_DISCOVERY_CLOSED",
      "创作者已明确结束本轮来源发现，不应继续追问。",
    );
  check(
    !["sufficient", "skipped", "uncertain", "exhausted"].includes(
      target.status,
    ) && target.answerInputIds.length < card.maxAnswers,
    "FOLLOWUP_CLOSED",
    "这个目标已经收敛、被跳过或达到本轮上限，不应重复追问。",
    { targetId },
  );
  if (card.stage === "methods" && targetId.startsWith("H")) {
    const objectId = `hypothesis.H${Number(targetId.slice(1))}`;
    check(
      state.artifacts[objectId] &&
        !confirmed(state, objectId, "accepted") &&
        !confirmed(state, objectId, "rejected"),
      "HYPOTHESIS_HANDLED",
      "该方法候选尚未生成或已处理完成。",
    );
  }
  if (card.stage === "methods" && !targetId.startsWith("H")) {
    const h = Object.values(state.artifacts).filter(
      (a) => a.kind === "hypothesis",
    );
    check(
      h.length >= 3 &&
        ...
        h.some((a) => confirmed(state, a.id, "accepted")),
      "METHOD_HYPOTHESES",
      "方法候选尚未完成校准。",
    );
    if (targetId.startsWith("E"))
      check(
        ["M01", "M02", "M03", "M04"].every((id) =>
          confirmed(state, `scenario.${id}`),
        ),
        "BASE_SCENARIOS",
        "先完成四个基础场景。",
      );
    check(
      !confirmed(state, `scenario.${targetId}`),
      "SCENARIO_HANDLED",
      "这个场景已经有效确认，不应重复采访。",
    );
  }
  if (targetId === "R00")
    check(
      STAGES.every((s) => bookConfirmed(state, s)),
      "REVISION_ENTRY",
      "先完成当前手册。",
    );
  if (card.stage === "service" && !["S00", "R00"].includes(targetId))
    check(
      state.serviceModelExplained && state.serviceMode,
      "SERVICE_INTRO",
      "先解释固定服务模式，并由创作者选择规划方式。",
    );
}
export function validateDelivery(result: WorkResult) {
  const d = result.delivery,
    mode = d.mode ?? "ordinary",
    length = [...d.text].length,
    questions = (d.text.match(/[?？]/g) ?? []).length;
  check(
    !(d.questionTargetId && d.confirmationObjectIds),
    "ONE_REPLY_TARGET",
    "每轮只保留一个问题或确认对象。",
  );
  check(
    questions === 0 || d.questionTargetId || d.confirmationObjectIds,
    "QUESTION_ID_REQUIRED",
    "对话中的问题必须绑定实际待回答对象，不能绕过采访计数。",
  );
    ...
    if (mode === "transition") {
      ...
      check(
        parts.length === 2 &&
          ...
          length <= 160,
        "TRANSITION_STYLE",
        "服务动线问法需要两段、两个问号、70—160字，第二段以“比如，”开头。",
      );
    } else
      check(
        questions === 1 && length <= (mode === "example" ? 180 : 100),
        "QUESTION_STYLE",
        "每轮一个核心问题；普通问法不超过100字，结合场景不超过180字。",
      );
  } else if (result.intent === "explain")
    check(
      !d.confirmationObjectIds &&
        questions === 0 &&
        length >= 80 &&
        length <= 220,
      "EXPLANATION_STYLE",
      "解释使用80—220字，不顺带提出采访问题或确认。",
    );
}
...
function canRatifyHypothesis(state: ProjectState, a: Artifact) {
...
export function continuationOptions(state: ProjectState, store: Store) {
  const questionTargets: string[] = [], confirmationObjects: string[] = [], preparation: string[] = [];
  ...
  for (const [id, card] of Object.entries(CARDS)) {
    if (card.stage !== state.stage) continue;
    try {
  ...
  for (const a of Object.values(state.artifacts)) {
    if (a.stage !== state.stage || a.unresolved.length) continue;
...
function validateContinuation(state: ProjectState, store: Store, result: WorkResult) {
  if (result.intent === "explain" || state.paused ||
    result.delivery.questionTargetId || result.delivery.confirmationObjectIds) return;
  const options = continuationOptions(state, store);
  check(
    !options.questionTargets.length && !options.confirmationObjects.length && !options.preparation.length,
    "NEXT_ACTION_REQUIRED",
    "访谈还有可推进内容，不能只回复已更新或已保留。请在本次 delivery 内接一个有绑定的下一问或版本确认；需要整理产物时先完成整理再提交。不要求用户说继续，不扩大确认范围。",
    { ...options, recovery: "修正当前工作结果后重试；不新增用户输入，不修改已经成功的前置工作。" },
  );
}
export function reduce(
  ...
  result: WorkResult,
  requestId: string,
): { state: ProjectState; delivery: Delivery; conversationOnly: boolean } {
  ...
  check(
    result.intent !== "explain" || !substantive,
    "EXPLANATION_MUTATION",
    "解释不能改动产物、确认、来源计划或采访状态。",
  );
  check(
    !result.artifacts?.some((a) =>
      result.confirmation?.target.objects.some((o) => o.id === a.id),
    ),
    "UNSEEN_CONFIRMATION",
    "同一句修改和确认先保存修改，新版展示后再确认。",
  );
  if (result.majorChange) {
    ...
    const cycleId = `cycle_${hash(input.id).slice(0, 24)}`,
      affected = [...new Set(result.majorChange.affectedTargets)];
    for (const targetId of affected) {
      if (/^D0[1-6]$/.test(targetId))
        impacted.add(`definition.${Number(targetId.slice(1))}`);
      else
        for (const a of Object.values(state.artifacts))
          if (a.stage === CARDS[targetId]?.stage) impacted.add(a.id);
    }
    check(
      affected.every((id) => CARDS[id]),
      "UNKNOWN_TARGET",
      "改向包含未知目标。",
    );
  ...
  const answeredDelivery = input.replyToDeliveryId
    ...
    : undefined;
  if (
    ["answer", "skip"].includes(result.intent) &&
    answeredDelivery?.question &&
    !state.answeredInputs.includes(input.id)
  ) {
    const { targetId, cycleId } = answeredDelivery.question,
      target = state.targets[targetId]!;
    check(
      target.cycleId === cycleId,
      "OLD_QUESTION",
      "输入指向旧访谈轮，请按修订或补充重新准备。",
    );
    target.answerInputIds.push(input.id);
    state.answeredInputs.push(input.id);
    if (result.intent === "skip") target.status = "skipped";
  ...
  for (const assessment of result.assessments ?? []) {
    ...
    check(
      CARDS[target.id]!.stage === base.stage ||
        result.intent === "revise" ||
        result.intent === "supplement",
      "STAGE_SCOPE",
      "跨阶段内容应保存为线索，不应提前完成采访。",
    );
    ...
    target.status =
      ...
      assessment.status !== "sufficient"
        ...
        : assessment.status;
    if (
      target.answerInputIds.length >= CARDS[target.id]!.maxAnswers &&
  ...
  for (const source of result.sourceVersions ?? []) {
    ...
    check(
      s.status === "ready" && s.chunks.length,
      "SOURCE_NOT_READY",
      "来源未完成解析，不能纳入正式知识。",
    );
    state.sources[s.id] = { version: s.version, kind: s.kind };
  }
  if (result.serviceModelExplained) state.serviceModelExplained = true;
  if (result.serviceMode) {
    check(state.serviceModelExplained, "SERVICE_INTRO", "先解释固定服务模式。");
    state.serviceMode = result.serviceMode;
  }
  if (result.intent === "pause") state.paused = true;
  if (result.intent === "resume") state.paused = false;
  if (result.confirmation) {
    const c = result.confirmation;
    inputEvidence(store, base, input, c.evidence);
    check(
      answeredDelivery?.confirmationTarget && store.shown(answeredDelivery.id),
      "CONFIRMATION_NOT_SHOWN",
      "没有可核对的已展示确认对象。请先展示确切版本。",
    );
    const shown = answeredDelivery.confirmationTarget;
    check(
      c.target.stage === shown.stage &&
        c.target.objects.every((o) =>
          shown.objects.some((s) => s.id === o.id && s.hash === o.hash),
        ),
      "CONFIRMATION_SCOPE",
      "确认不能扩大到未展示的对象。",
    );
    if (c.target.scope === "booklet")
      check(
        shown.scope === "booklet" &&
          hash(c.target.objects) === hash(shown.objects),
        "CONFIRMATION_SCOPE",
        "整册确认需要明确展示完整手册。",
      );
    for (const object of c.target.objects) {
      ...
      check(
        a?.hash === object.hash && !a.unresolved.length,
        "CONFIRMATION_STALE",
        "对象已修改或仍有未决事项，不能确认。",
      );
      if (a.kind === "chapter")
        check(
          canDraft(state, store, a.stage),
          "STAGE_GATES",
          "当前手册硬门槛未通过。",
        );
      if (a.kind === "blueprint")
        check(
          serviceGates(state).every((g) => g.pass),
          "SERVICE_GATES",
          "服务蓝图还有未通过的规则。",
          { gates: serviceGates(state) },
        );
      check(
        a.kind === "hypothesis"
  ...
  const changed = new Set<string>(impacted);
  for (const proposal of result.artifacts ?? []) {
    const patch = structuredClone(proposal);
    check(
      STAGES.indexOf(patch.stage) <= STAGES.indexOf(state.stage),
      "STAGE_SCOPE",
      "上游手册尚未确认，不能生成后续阶段产物。",
    );
    if (patch.kind === "chapter") {
      check(
        bookIds(patch.stage).includes(patch.id),
        "CHAPTER_SCHEMA",
        "手册章节编号不符合固定结构。",
      );
    ...
    if (patch.kind === "transition") {
      ...
      check(
        ["trigger", "rightsChange", "dataInheritance", "message"].every(
          (k) =>
            typeof patch.data?.[k] === "string" && String(patch.data[k]).trim(),
        ),
        "TRANSITION_INCOMPLETE",
        "路径需要时点、权益、历史继承和表达四项具体内容。",
      );
    }
    if (patch.kind === "hypothesis")
      check(
        patch.stage === "methods" && /^hypothesis\.H[1-4]$/.test(patch.id),
        "HYPOTHESIS_SCHEMA",
        "方法候选使用 H1—H4，必须为方法阶段。",
      );
    if (patch.kind === "scenario") {
      check(
        patch.stage === "methods" &&
          /^scenario\.(M0[1-4]|E0[1-3])$/.test(patch.id),
        "SCENARIO_SCHEMA",
        "只能使用固定四基础、三拓展维度。",
      );
      if (patch.id.startsWith("scenario.M"))
        check(
          patch.evidence.some((r) => r.type === "input"),
          "USER_ANSWERS_FIRST",
          "基础场景先保存用户实际处理答案。",
        );
    }
    if (patch.kind === "blueprint") {
      check(
        patch.id === "service.blueprint" && patch.stage === "service",
        "BLUEPRINT_SCHEMA",
        "服务蓝图身份不匹配。",
      );
      check(
        serviceRulesMatch(patch.data ?? {}),
        "FIXED_SERVICE_RULES",
        "服务蓝图须使用本轮 platformRules：免费范围由创作者定义、付费AI对话不限次数、维持期保留基础对话。旧规则仅供历史读取，修订时采用当前规则并重新展示确认。",
      );
      ...
      if (patch.data?.billingCycle !== undefined)
        check(subscriptionPeriod(patch.data.billingCycle), "SUBSCRIPTION_PERIOD",
          "订阅时长使用 {count:正整数,unit:day|week|month|year}；未讨论时可省略并保留待定，不能用交付频率代替订阅时长。旧 monthly 兼容为1个月。");
      const checks = (patch.data?.semanticChecks ?? {}) as Record<
        string,
        { evidence: Ref[] }
      >;
    ...
    for (const ref of [...patch.evidence, ...patch.dependencies])
      if (ref.type === "source")
        check(
          state.sources[ref.id],
          "SOURCE_NOT_ADOPTED",
          "引用的来源必须先绑定真实可用的正式版本。",
          { id: ref.id },
        );
    const requiredUpstream: Record<Stage, Stage[]> = {
      definition: [],
    ...
    for (const stage of requiredUpstream[patch.stage])
      check(
        patch.dependencies.some(
          (ref) =>
            ref.type === "artifact" && state.artifacts[ref.id]?.stage === stage,
        ),
        "DEPENDENCY_REQUIRED",
        "下游产物必须记录实际使用的已确认上游依据。",
        { artifact: patch.id, requiredStage: stage },
      );
    for (const ref of patch.dependencies)
      if (ref.type === "artifact")
        check(
          confirmed(state, ref.id) || confirmed(state, ref.id, "accepted"),
          "UNCONFIRMED_DEPENDENCY",
          "下游依据须来自已确认内容。",
        );
    const digest = hash(patch);
    if (state.artifacts[patch.id]?.hash !== digest) changed.add(patch.id);
    state.artifacts[patch.id] = {
      ...patch,
      hash: digest,
      revision:
        state.artifacts[patch.id]?.hash === digest
    ...
    if (
      ...
      check(
        result.intent === "answer" &&
          answeredDelivery?.question?.targetId === patch.id.slice(9),
        "SCENARIO_ANSWER_SCOPE",
        "只能忠实记录用户刚回答的基础场景。",
      );
  ...
  let confirmationTarget: ConfirmationTarget | undefined;
  if (d.confirmationObjectIds) {
    ...
    check(
      !artifacts.some((a) => a!.kind === "hypothesis") ||
        artifacts.length === 1,
      "HYPOTHESIS_ONE_AT_A_TIME",
      "方法候选一次只展示并校准一条，请选择当前候选，保留其他候选待后续讨论。",
    );
    if (artifacts[0]!.kind === "hypothesis") {
      ...
      if (state.targets[targetId]?.status === "sufficient") {
        ...
        check(!candidate.unresolved.length && !state.targets[targetId]!.gaps.length,
          "HYPOTHESIS_UNRESOLVED", "方法候选仍有未决事项，不能请求正式确认。");
    ...
    check(
      artifacts.every((a) => a!.stage === stage),
      "CONFIRMATION_SCOPE",
      "一轮不能跨阶段确认。",
    );
    ...
    if (scope === "booklet")
      check(
        hash([...d.confirmationObjectIds].sort()) ===
          hash(bookIds(stage).sort()),
        "BOOKLET_INCOMPLETE",
        "整册展示必须包含所有固定章节。",
      );
    confirmationTarget = {
  ...
  check(
    !state.paused || !questionTargetId,
    "PAUSED",
    "创作者已暂停，本轮不能继续采访。",
  );
  if (questionTargetId) canAsk(state, store, questionTargetId);
