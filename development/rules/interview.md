...
Skill 0.2.3 延续 Codex、Claude Code、WorkBuddy 的首发目标，运行环境要求为 Node.js 22.13+，已移除 macOS 硬限制。提供跨平台 Node 入口，各宿主与系统组合仍需实机验收；Windows 的入口准备不代表其完整流程已支持或验收。默认采用“宿主主对话采访＋浏览器只读预览＋对话修正”；宿主内嵌预览与原生标注仅在实际可用时作为增强，不作为访谈正常进行的前提。用户明确改向时可对实际受影响目标开启新的访谈修订轮，具体计数规则见第 2.2 节。
...
首版在 Codex、Claude Code、WorkBuddy 中统一使用主对话采访、浏览器只读预览和对话修正；Node 入口不再限制 macOS，实际宿主与系统组合分别验收。预览与对话展示同一业务状态；预览没有编辑或确认控件。浏览器展示访谈结构、生成内容、待确认 Booklet 和原有服务模式样式图。内嵌页面、原生标注及完整元数据仅在宿主实际支持并验证后使用，缺少它们不阻塞默认体验。只有页面和入口确实可用时才能据此引导创作者，不能仅凭生成地址就宣称已经打开。访谈内容、问题次序和确认规则不随宿主界面改变。

### 11.4 必须覆盖的行为验收

| 输入或场景 | 必须表现 |
| --- | --- |
| 三宿主的首版标准体验 | 在明确记录的宿主与系统组合中完成主对话采访、浏览器只读预览和对话修正；没有原生标注时继续使用对话，不另加确认门槛，不以入口存在代替实机验收 |
| 来源类型与处理覆盖 | 纯文本、选定历史可见消息与文本 Skill 由本地整理归档；需识别或转换的文件、网页、扫描件和音视频使用宿主真实可用工具，缺少工具或仅有片段时保留缺口，不假装完整读取，不要求单个项目交齐所有类型 |
