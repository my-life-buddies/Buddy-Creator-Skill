# Buddy Creator Skill 0.2.1

本版本将 Buddy 的主对话采访、来源处理、工作项恢复与只读预览作为完整 skill 分发。四册确认后生成本地成果，不包含模拟开发交接和小红书账号采集。

## 构建与安装

开发目录运行 `npm run pack:skill`。输出 release/buddy-creator/ 与带 SHA-256 的版本 ZIP。包中包含编译后的工具、按需读取的规则、前端静态文件和锁定的生产依赖；安装后不需 npm install 或前端构建。Node.js 22.13+ 由宿主所在机器提供。音视频仍依赖 macOS 系统资源与 FFmpeg。发布清单记录构建架构及未携带的可选依赖；其他 macOS 架构的原生解析能力须单独验证，不能只凭 JavaScript 可运行宣称全部支持。
...
自动验证覆盖四阶段完整流程、确认约束、局部恢复、来源采集排除、历史资料读取、独立安装目录中的启动与预览。实际宿主的 skill 发现、导入、内嵌预览、自然语言采访须分别验收；手动 --host 参数的启动测试不等于三个宿主的真实体验验收。
